...
export default function PortfolioPage({ params }) {
  ...
  const fetchProjects = async () => {
    try {
      const res = await fetch(`${process.env.NEXT_PUBLIC_WP_API_BASE}/posts?category=projects`);
      const data = await res.json();
      const filteredProjectsBlogs = filterBlogs({
        blogs: data.posts,
        author: decodedParam,
      })
      setProjects(filteredProjectsBlogs || []);
    } catch (error) {
      console.error("Error fetching projects in portfolio:", error);
    }
  };
  const fetchBlogs = async () => {
    try {
      const res = await fetch(`${process.env.NEXT_PUBLIC_WP_API_BASE}/posts?category=blogs`);
      const data = await res.json();
      const filteredProjectsBlogs = filterBlogs({
        blogs: data.posts,
        author: decodedParam,
      ...
      setBlogs(filteredProjectsBlogs || []);
    } catch (error) {
      console.error("Error fetching projects in portfolio:", error);
    }
  };
                ...
                <div key={index} className="relative flex items-start mb-8 pb-8 border-b border-gray-200 last:border-b-0">
                  ...
                  <div className="flex-1 min-w-0">
                    ...
                    <p className="text-sm text-gray-500 mb-3">
                      {moment(exp?.date?.start).format("MMM YYYY")} -
                      {exp?.date?.status === "Present"
                        ? "Present"
                        : moment(exp?.date?.end).format("MMM YYYY")}
                    </p>
